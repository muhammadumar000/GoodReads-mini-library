      ...
      return (
        <Box display='flex' justifyContent="center" alignItems="center" gap='1rem'>
          ...
          <Tooltip title={userData[0].userName}>
            <Avatar sx={{bgcolor:'#52ab98',cursor:'pointer',marginRight:'1rem'}}  src="/broken-image.jpg" />
          </Tooltip>
        </Box>
      );
    }
  ...
  return (
    <Box sx={{ flexGrow: 1,width:'100vw' }}>
      <AppBar position="static" sx={{ bgcolor: "#2b6777" }}>
        ...
        anchorEl={anchorEl}
        open={open}
        PaperProps= {{sx: {marginTop:'1rem',width:'10rem',backgroundColor: '#2b6777'}}}
        onClose={handleClose}
        MenuListProps={{
          'aria-labelledby': 'basic-button',
        }}
      >
        <MenuItem  onClick={handleClose} ><Linkd to="/">Home</Linkd></MenuItem>
        <MenuItem onClick={handleClose} ><Linkd to="/login">LogIn</Linkd></MenuItem>
        <MenuItem onClick={handleClose} ><Linkd to="/signup">SignUp</Linkd></MenuItem>
      </Menu>
          <Box ml={1} display="flex" sx={{ flexBasis: "100%" }}>
            <Typography variant="h5" sx={{display:{xs:'none',sm:'block'}}}>Goodreads</Typography>
            <LocalLibrary size='large' sx={{display:{xs:'block',sm:'none'}}} />
            <Box
              mx="auto"
              display="flex"
              alignItems="center"
